...
function viewFeedbackWindow(feedbackid) {
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-user-alt"></i> REPORTE</span>`)
	} else if (data.category === 'question') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> PREGUNTA</span>`)
	} else {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-bug"></i> ERROR</span>`)
	}
}
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> REPORTE</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-question'></i> PREGUNTA</span>"
		} else {
			category = "<span><i class='fas fa-bug'></i> ERROR</span>"
		}
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>REPORTE #${feedback.feedbackid}</th>
					<td>${category}</td>
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("¡BUEN TRABAJO! AHORA PUEDES RELAJARTE UN POCO.");
		$(".feedbackTable .card-header").css('border-bottom', 'none');
		$(".feedbackTable .card-header").css('border-bottom-left-radius', '15px');
		$(".feedbackTable .card-header").css('border-bottom-right-radius', '15px');
		$(".feedbackTable .card-body").hide(); 
	} else {
		$(".feedback_subtitle").html("¡OYE! PARECE QUE TIENES ALGO DE TRABAJO POR HACER...");
		$(".feedbackTable .card-header").css('border-bottom', '3px solid #5ba6d6');
